...
void Map::AddOffice(Office office) {
  ...
  Office& added_office = offices_.emplace_back(std::move(office));
  
  try {
    warehouse_id_to_index_.emplace(added_office.GetId(), index);
  } catch (...) {
    offices_.pop_back();
    throw;
  }
...
void tag_invoke(json::value_from_tag, json::value& jv, const Road& road) {
  if (road.IsHorizontal()) {
    jv = {
      {ROAD_XO, json::value_from(road.GetStart().x)},
...
Road tag_invoke(json::value_to_tag<Road>, const json::value& jv) {
  ...
  try {
    Coord end_x = json::value_to<int>(jv.as_object().at(ROAD_X1));
    return Road(Road::HORIZONTAL, start, end_x);
  } catch (...) {
    Coord end_y = json::value_to<int>(jv.as_object().at(ROAD_Y1));
...
void tag_invoke(json::value_from_tag, json::value& jv, const Building& building) {
  jv = {
    {BUILDING_X, json::value_from(building.GetBounds().position.x)},
...
void tag_invoke(json::value_from_tag, json::value& jv, const Office& office) {
  jv = {
    {OFFICE_ID, json::value_from(*(office.GetId()))},
...
void tag_invoke(json::value_from_tag, json::value& jv, const Map& map) {
  jv = {
    {MAP_ID, json::value_from(*(map.GetId()))},

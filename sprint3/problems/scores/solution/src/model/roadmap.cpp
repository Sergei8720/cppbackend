...
void Roadmap::AddHorizontalRoad(const Road& road, int64_t scaledOffset) {
    ...
    start = start * SCALE_FACTOR_OF_CELL - scaledOffset;
    end = end * SCALE_FACTOR_OF_CELL + scaledOffset;
    auto y = road.GetStart().y * SCALE_FACTOR_OF_CELL;
    for(auto x = start; x <= end; ++x) {
        for(auto i = -(scaledOffset); i <= scaledOffset; ++i) {
            matrix_map_[x][y + i].insert(roads_.size() - 1);
        }
    }
}

void Roadmap::AddVerticalRoad(const Road& road, int64_t scaledOffset) {
    ...
    for(auto y = start; y <= end; ++y) {
        for(auto i = -scaledOffset; i <= scaledOffset; ++i) {
            matrix_map_[x + i][y].insert(roads_.size() - 1);
        }
    }
}
...
void Roadmap::AddRoad(const Road& road) {
    ...
    if(road.IsHorizontal()) {
        AddHorizontalRoad(road, SCALLED_OFFSET);
    } else {
        AddVerticalRoad(road, SCALLED_OFFSET);
    }
};
...
std::tuple<geom::Point2D, Velocity> Roadmap::GetValidMove(const geom::Point2D& old_position,
    ...
    if(end_roads){
        if(!IsValidPosition(matrix_map_.at(end_roads.value().x).at(end_roads.value().y),
    ...
    if(dest && IsValidPosition(matrix_map_.at(dest.value().x).at(dest.value().y), potential_new_position)) {
    ...
    if(old_velocity.vx != 0) {
        ...
        for(index = start_coord.x; index != end_x; index += direction) {
            if(ValidateCoordinates({index, start_coord.y}) &&
                IsCrossedSets(matrix_map_.at(start_coord.x).at(start_coord.y),
                                matrix_map_.at(index).at(start_coord.y))) {
        ...
        for(index = start_coord.y; index != end_y; index += direction) {
            if(ValidateCoordinates({start_coord.x, index}) &&
                IsCrossedSets(matrix_map_.at(start_coord.x).at(start_coord.y),
                                matrix_map_.at(start_coord.x).at(index))) {
...
std::optional<const Roadmap::MatrixMapCoord> Roadmap::GetCoordinatesOfPosition(const geom::Point2D& position) {
    ...
    if(matrix_map_.contains(x_index)) {
        if(matrix_map_.at(x_index).contains(y_index)) {
...
bool Roadmap::ValidateCoordinates(const MatrixMapCoord& coordinates) {
    if(matrix_map_.contains(coordinates.x)) {
        return matrix_map_.at(coordinates.x).contains(coordinates.y);
    }
    return false;
};
...
const geom::Point2D Roadmap::GetFarestPoinOfRoute(const MatrixMapCoord& roads_coord,
    ...
    for(auto road_ind : matrix_map_.at(roads_coord.x).at(roads_coord.y)) {
...
std::tuple<double, double, double, double> Roadmap::CalculateRoadBounds(const Road& road) const {
    ...
    return std::make_tuple(start_x, end_x, start_y, end_y);
}

bool Roadmap::IsPositionWithinBounds(double pos, double start, double end) const {
    return (pos > start || std::abs(pos - start) < EPSILON) &&
           (pos < end || std::abs(pos - end) < EPSILON);
}
...
bool Roadmap::IsValidPositionOnRoad(const Road& road, const geom::Point2D& position) const {
    auto [start_x, end_x, start_y, end_y] = CalculateRoadBounds(road);
    
    return IsPositionWithinBounds(position.x, start_x, end_x) &&
           IsPositionWithinBounds(position.y, start_y, end_y);
};
